...
Refer to [metal_sdk/example/] python scripts.
Refer to [metal_sdk/README.md].

## Use metal_ros2(基于python sdk封装的ros2, 推荐入门使用, 不再需要自己调用接口写程序)
...
### 单臂使用（末端：夹爪示教器2合1）
  ...
  单臂重力补偿模式(采集数据时可用):
  ```sh
  ros2 launch metal_controller one_master.launch
  ```

  单臂控制模式(关节位置或末端位姿控制时可用):
  ```sh
  ros2 launch metal_controller single_arm_control.launch
  ```
...
### 双臂使用 （末端：夹爪示教器2合1）
  ...
  双臂重力补偿模式(采集数据时可用):
  ```sh
  ros2 launch metal_controller two_master.launch
  ```
  双臂控制模式(关节位置或末端位姿控制时可用):
  ```sh
  ros2 launch metal_controller two_arm_control.launch
  ```
...
### 一主一从使用（两条臂，一个末端是示教器， 一个末端是夹爪）
  ...
  主从摇操模式(采集数据时可用):
  ```sh
  ros2 launch metal_controller one_master_slave.launch
  ```
...
### 两主两从使用 （四条臂，两个末端是示教器， 两个末端是夹爪）
  ...
  主从摇操模式(采集数据时可用):
  ```sh
  ros2 launch metal_controller two_master_slave.launch
  ```
